...
graph TB
    subgraph Frontend["⚛️ React 前端"]
        User["用户端<br/>智能对话 · 文档预览 · 历史管理"]
        Admin["管理端<br/>仪表盘 · 知识库管理 · Agent执行记录 · 知识巡检 · 自动报表"]
    end

    subgraph Backend["☕ Spring Boot 后端"]
        Auth["用户认证 (JWT)"]
        Session["会话管理"]
        Cache["多级缓存<br/>Caffeine + Redis"]
        DocMgr["文档管理"]
    end

    subgraph AgentLayer["🐍 Python AI 服务 — 五层 Agent 架构"]
        L1["Interface 层<br/>FastAPI 路由 · HTTP/SSE 接口"]
        L2["Orchestrator 层<br/>Planner 规划 · Executor 执行 · State 状态"]
        L3["Tool 层<br/>ToolRegistry 统一注册/调用/超时/重试"]
        L4["Memory 层<br/>Redis 短期 · MongoDB 会话 · MySQL 持久化"]
        L5["Evaluation 层<br/>检索充分性判断 · 结果质量评估"]
        L1 --> L2 --> L3 --> L4 --> L5
    end
    ...
    Frontend -->|HTTP / SSE · JWT认证| Backend
    Backend -->|REST API| AgentLayer
    AgentLayer --> Storage
```
